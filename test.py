...
def Pre_emphasis(signal):
    ...
    plt.savefig("Pre-Emphasis.png")
    plt.show()
    return axis_x,emphasized_signal
...
if __name__ == '__main__':
    ...
    sample_rate_B, signal_B = load(os.path.join('C:\\Users\\USER\\Desktop\\flask-templete\\static\\audio\\B.wav'))

    axis_x_A,emphasized_signal_A= Pre_emphasis(signal_A)
    axis_x_B,emphasized_signal_B= Pre_emphasis(signal_B)

    frames_A=Split(sample_rate_A,emphasized_signal_A)
    frames_B=Split(sample_rate_B,emphasized_signal_B)
    ...
    # Reshape mfccs_B to match the dimensions of mfccs_A
    min_length = min(mfcc_nor_A.shape[1], mfcc_nor_B.shape[1])
    mfccs_A1 = mfcc_nor_A[:, :min_length].reshape(min_length, min_length)
    mfccs_B1 = mfcc_nor_B[:, :min_length].reshape(min_length, min_length)
    score=compute_similarity_score(mfccs_A1, mfccs_B1)
    print(score)